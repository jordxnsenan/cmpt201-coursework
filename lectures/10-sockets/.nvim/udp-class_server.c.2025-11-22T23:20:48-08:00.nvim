...
int main(void) {
  ...
  if (remove("da_udp_socket") == -1 && errno != ENOENT) {
    perror("remove");
    exit(EXIT_FAILURE);
  }
  // bind
  ...
  snprintf(sockstruct.sun_path, 108,
           "da_udp_socket"); // opens local socket named socket_fun
